...
local function getModuleInfo(modName)
    ...
    while true do
        ...
        if resource and not src:find('^@@srs_lib/libs/require') then
...
function srs.lib.load(filePath, env)
...
function srs.lib.loadJson(filePath)
    if type(filePath) ~= 'string' then
        error(("file path must be a string (received '%s')"):format(filePath), 2)
    end
...
---@param modName string
---@return unknown
function srs.lib.require(modName)
