...
function srs.lib.raycast.fromCoords(coords, destination, flags, ignore)
    local ped = PlayerPedId()
    local handle = StartShapeTestLosProbe(coords.x, coords.y, coords.z, destination.x, destination.y,
        destination.z, flags or 511, ped, ignore or 4)
...
function srs.lib.raycast.fromCamera(flags, ignore, distance)
    local coords = GetFinalRenderedCamCoord()
    local destination = coords + getForwardVector() * (distance or 10)

    return srs.lib.raycast.fromCoords(GetFinalRenderedCamCoord(), destination, flags, ignore)
end

---@deprecated
srs.lib.raycast.cam = srs.lib.raycast.fromCamera

return srs.lib.raycast
