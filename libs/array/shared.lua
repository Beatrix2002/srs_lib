...
srs.lib.array = srs.lib.class('Array')
...
---@private
function srs.lib.array:constructor(...)
    local arr = { ... }
...
---@private
function srs.lib.array:__newindex(index, value)
...
---@return Array
function srs.lib.array:from(iter)
    local iterType = type(iter)

    if iterType == 'table' then
        return srs.lib.array:new(table_unpack(iter))
    end

    if iterType == 'string' then
        return srs.lib.array:new(string.strsplit('', iter))
    end

    if iterType == 'function' then
        local arr = srs.lib.array:new()
        local length = 0
...
---@param index number
---@return unknown
function srs.lib.array:at(index)
    if index < 0 then
        index = #self + index + 1
    end
...
function srs.lib.array:fill(value, start, endIndex)
    local length = #self
    start = start or 1
...
---@param testFn fun(element: unknown): boolean
function srs.lib.array:filter(testFn)
    local newArr = {}
    local length = 0
...
---@param testFn fun(element: unknown): boolean
---@param last? boolean
function srs.lib.array:find(testFn, last)
...
---@param testFn fun(element: unknown): boolean
---@param last? boolean
function srs.lib.array:findIndex(testFn, last)
    local a = last and #self or 1
    local b = last and 1 or #self
    local c = last and -1 or 1
...
---@param value unknown
---@param last? boolean
function srs.lib.array:indexOf(value, last)
    local a = last and #self or 1
    local b = last and 1 or #self
    local c = last and -1 or 1
...
function srs.lib.array:forEach(cb)
    for i = 1, #self do
        cb(self[i])
    end
end
...
function srs.lib.array:includes(element, fromIndex)
...
function srs.lib.array:join(seperator)
...
---@return T
function srs.lib.array:reduce(reducer, initialValue, reverse)
    local length = #self
...
function srs.lib.array:reverse()
    local i, j = 1, #self
...
---@param start? number
---@param finish? number
function srs.lib.array:slice(start, finish)
    local length = #self
    start = start or 1
...
function srs.lib.array:toReversed()
    local reversed = srs.lib.array:new()
...
---@param ... any
function srs.lib.array:unshift(...)
    local elements = { ... }
    local length = #self
...
---@param tbl ArrayLike
---@return boolean
function srs.lib.array.isArray(tbl)
    local tableType = table_type(tbl)

    if not tableType then return false end

    if tableType == 'array' or tableType == 'empty' or srs.lib.array.instanceOf(tbl, srs.lib.array) then
        return true
    end
