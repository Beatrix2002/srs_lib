...
RegisterNetEvent(cbEvent:format(srs.cache.resource), function(key, ...)
...
local function triggerClientCallback(_, event, playerId, cb, ...)
    ...
    TriggerClientEvent('srs_lib:validateCallback', playerId, event, srs.cache.resource, key)
    TriggerClientEvent(cbEvent:format(event), playerId, srs.cache.resource, key, ...)
...
function srs.callback.await(event, playerId, ...)
...
---@param cb function
---@diagnostic disable-next-line: duplicate-set-field
function srs.callback.register(name, cb)
    event = cbEvent:format(name)

    srs.setValidCallback(name, true)
